...
dict_version = { 19:"35", 20:"36", 21:"37", 22:"38"}
...
def run_shell_command(command_line):
    ...
    try:
        ...
        log_txt = process_output.decode('utf-8', errors='ignore')
...
def finalizexml(fpath_xml,fpath_gsm,version,reportlevel=1, verbosityLevel=1):
    """
    ...
    cmd = '%s finalizelibrary -l UTF8 %s -verbose %d %s %s' % (path_LP_XMLConverter, checkall_txt,verbosityLevel,fpath_xml, fpath_gsm)
...
def gsm2xml_batch(path_xml,path_gsm,version):
    ...
    path_xml = '"' + path_xml + '"'
    path_LP_XMLConverter = getconverter(version)
    cmd = u'%s l2x -l UTF8 %s %s' % (path_LP_XMLConverter, path_gsm , path_xml)
    rezult = run_shell_command(cmd)
    return rezult
