...
converterName = "LP_XMLConverter.exe"
cygwinpath = os.path.abspath (os.path.join ('..', '..', 'cygwin'))

def FillDLLDependencyList (sourceFolder, fileList):
	success = True;
	
	converterFile = os.path.join (sourceFolder, converterName)
	
	savedir = os.getcwd ()
	os.chdir (sourceFolder)
	cygcheckproc = subprocess.Popen([os.path.join (cygwinpath, 'cygcheck.exe'), converterFile],
							stdout=subprocess.PIPE)
	grepproc = subprocess.Popen([os.path.join (cygwinpath, 'grep.exe'), '-F', '-i', sourceFolder],
							stdin=cygcheckproc.stdout,
							stdout=subprocess.PIPE)
	stdout, stderr = grepproc.communicate()
	os.chdir (savedir)
...
def CopyFiles (sourceFolder, destFolder, rsrcFolderName, copiedFiles):
	...
	if not FillDLLDependencyList (sourceFolder, fileList):
		return False
