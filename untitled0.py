...
class gdl_gsm(object):
    ...
    def _write_(self):
        ...
        self.root.unlink()
        convert.xml2gsm(fname_gsm, fname_xml)
        dpr('GSM файл создан')
...
path_work = 'G:\\Cloud\\Google Диск\\Share\\pygdl\\'

gsmname = '14'
templatename = 'common'

fname_template_xml = path_work + 'template\\' + templatename + '.xml'
fname_template_gsm = path_work + 'template\\' + templatename + '.gsm'
fname_gsm = path_work + 'test\\gsm\\' + gsmname +'.gsm'
fname_xml = path_work + 'test\\xml\\' + gsmname +'.xml'

#convert.gsm2xml(fname_template_gsm, fname_template_xml)
    
g = gdl_gsm(fname_gsm, fname_xml, fname_template_xml)

g.define_style("текст", 'ISOCPEUR' , 'Shrift_razm', 5, 'Shrift_tip')
g.set_style("текст")
g.param('CALL "Общий шаблон элемента" parameters all')
